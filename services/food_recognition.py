...
Пользователь прислал коррекцию/уточнение (может содержать несколько пунктов изменений):
"{correction_text}"

Задача:
1. Внимательно проанализируй ВСЕ пункты коррекции, которые указал пользователь
2. Понять, что именно пользователь хочет изменить (название, ингредиенты, вес порции, КБЖУ)
3. Применить ВСЕ изменения из коррекции одновременно
4. Обновить данные о блюде с учетом всех коррекций
5. Пересчитать КБЖУ максимально точно с учетом всех изменений

Важно:
- Если указано несколько изменений (например, "не 100 грамм, а 50" и "добавь сыр 30 грамм") - примени ВСЕ
- Если указано заменить ингредиент - замени его полностью
- Если указано добавить ингредиент - добавь его в список
- Если указано убрать ингредиент - удали его из списка
- Если указано изменить вес/количество - пересчитай КБЖУ пропорционально
